...
//   e.preventDefault()
//   const value = parseInt(amount.value) // parseInt used to "convert string to number"
//   console.log(value)
//   // Generate Random paragraph
//   const random = Math.floor(Math.random() * text.length);

//   // If any value from these 3 "empty", "less than 0" or "greater than 9" is "true" then generate paragraph
//   if(isNaN(value) || value <= 0 || value > 9){
//     result.innerHTML = `<p class="result>${text[random]}</p>`
//   }
//   else{
//     let tempText = text.slice(0, value) // (0, value) means start from '0' and whatever value you enter it will generate that paragraph
...
form.addEventListener('submit', function(e){
  e.preventDefault()
    ...
    // else if user enter more than 9 paragraph
    else{
      let tempText = text.slice(0, value) //slick syntax is (0, 1)
      tempText = tempText.map(function(item){
        return `
        <p class="result">${item}</p>
        `
